# -*- coding: utf-8 -*-
"""
experiments/terminal_reward_severity_sweep.py

Compare all 3 RL methods (Group RL cold, Group RL warm, Node RL)
with and without terminal reward across 4 severity scenarios,
plus OC-Guided as reference.

Scenarios: baseline (dY=0.27), moderate (dY=0.40),
           severe (dY=0.50), critical (dY=0.65)
"""
...
def run_sweep(out_dir='results/terminal_severity_sweep', n_eval=10):
    ...
    states_opt, ctrl_opt, _ = solve(params_base, init_pattern='hcp')
    ax, ay, az = allocations_from_solution(states_opt, ctrl_opt)
    ...
    scenarios = [
        {'label': 'baseline', 'pY': 0.2, 'dY': 0.27},
        {'label': 'moderate', 'pY': 0.3, 'dY': 0.40},
        {'label': 'severe',   'pY': 0.4, 'dY': 0.50},
        {'label': 'critical', 'pY': 0.5, 'dY': 0.65},
    ]
    ...
    for sc in scenarios:
        ...
        all_results.append({
            'scenario': sc['label'], 'dY': sc['dY'],
            'method': 'OC-Guided', 'terminal_scale': '-',
        ...
        for ts in terminal_scales:
            ...
            all_results.append({
                'scenario': sc['label'], 'dY': sc['dY'],
                'method': 'Group RL cold', 'terminal_scale': ts,
            ...
            all_results.append({
                'scenario': sc['label'], 'dY': sc['dY'],
                'method': 'Group RL warm', 'terminal_scale': ts,
            ...
            all_results.append({
                'scenario': sc['label'], 'dY': sc['dY'],
                'method': 'Node RL', 'terminal_scale': ts,
    ...
    print("\n" + "=" * 70)
    print("TERMINAL REWARD × SEVERITY SWEEP")
    print("=" * 70)
    ...
    csv_path = os.path.join(out_dir, 'results_terminal_severity.csv')
